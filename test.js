...
function HL7Date(timestamp) {
    return Format(timestamp.getFullYear(),4)+Format(timestamp.getMonth()+1,2)+Format(timestamp.getDate(),2)+Format(timestamp.getHours(),2)+Format(timestamp.getMinutes(),2)+Format(timestamp.getSeconds(),2);
}

function main(message) {
    let hl7 = new HL7(message);

    hl7.transform(err => {
        if (err) throw err;

        console.info("Received:");
        console.info(hl7.build());
    });
    ...
    ack.set('MSH', {
        ...
        'MSH.5': hl7.get("MSH.3"),
        'MSH.6': hl7.get("MSH.4"),
        'MSH.7': HL7Date(timestamp),
        'MSH.8': '',
        'MSH.9': {
            'MSH.9.1': hl7.get("MSH.9.1"),
            'MSH.9.2': hl7.get("MSH.9.2"),
            'MSH.9.3': 'ACK'
        },
        'MSH.10': hl7.get("MSH.10") + '-ACK',
